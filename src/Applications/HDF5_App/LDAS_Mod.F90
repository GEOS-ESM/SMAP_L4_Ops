...
!     The name of the coordinate file is
!     conveyed through an environment variable.
!     =========================================

      if (TILES%numTiles .eq. 0) then

        call getenv('LDAS_TILE_COORD_FILE',fname)
        if (len_trim(fname) .eq. 0) rc = 1
        if (isError(this,2,rc) .ne. 0) return

        rc = TILEread(fname,TILES)
        if (isError(this,3,rc) .ne. 0) return

!       Set tile to grid mapping

        rc = GPDgetGrid(grid)
        rc = TILE2grid(TILES,grid)
...
!     ================================
      ...
      if (associated(element%f32)) then
        ...
        read(IUNIT,iostat=rc) f32
        if (isError(this,2,rc,arg=element%info%name) .ne. 0) return

        where (f32 .lt. -9998.0) f32 = SDS_F32_FILL
        where (f32 .gt. 9.9e+14) f32 = SDS_F32_FILL

        N_data = size(f32)
        fill = SDS_F32_FILL

        N_shave = element%info%shave_bits
        if (N_shave .gt. 0) then
          print *, 'Applying bit shaving to: ', trim(element%info%name)
          rc = ShaveMantissa32(f32, f32, N_data, N_shave, .true., fill, N_data)
        endif
      ...
      elseif (associated(element%f64)) then
        ...
        allocate(f64(TILES%numTiles),stat=rc)
        if (isError(this,1,rc) .ne. 0) return

        element%f64 = SDStransfer(element%info%fill_value,SDS_F64_FILL)

        read(IUNIT,iostat=rc) f64
        if (isError(this,2,rc,arg=element%info%name) .ne. 0) return

        where (f64 .lt. -9998.0) f64 = SDS_F64_FILL
        where (f64 .gt. 9.9e+14) f64 = SDS_F64_FILL
        element%f64(TILES%gridmap) = f64
      ...
      elseif (associated(element%i32)) then
        ...
        read(IUNIT,iostat=rc) i32
        if (isError(this,2,rc,arg=element%info%name) .ne. 0) return

        where (i32 .lt. -9998) i32 = SDS_I32_FILL
      ...
      elseif (associated(element%u32)) then
        ...
        where (i32 .lt. -9998) i32 = SDS_U32_FILL
...
!     ==================
      ...
      elseif (isYProjCoord(element)) then

        element%info%rank      = 1
        element%info%dims(1)   = rows
        element%info%dims(2)   = 1
        element%info%chunks    = 0
        element%info%chunks(1) = rows

        rc = SDSallocate(element,real_size=64)
        if (isError(this,1,rc) .ne. 0) return

        rc = GPDgetYProj(element%f64)
      ...
      elseif (isXProjCoord(element)) then
        ...
        element%info%rank      = 1
        element%info%dims(1)   = cols
        element%info%dims(2)   = 1
        element%info%chunks    = 0
        element%info%chunks(1) = cols

        rc = SDSallocate(element,real_size=64)
        if (isError(this,1,rc) .ne. 0) return

        rc = GPDgetXProj(element%f64)
