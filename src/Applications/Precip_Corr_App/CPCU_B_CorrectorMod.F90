      ...
      real, parameter    :: LAT_TRANS_MIN = 42.5
      real, parameter    :: LAT_TRANS_MAX = 62.5
...
!     Interpolate the CPCU data to the
!     model grid.
!     ================================

      print *, 'Interpolating CPCU data to model grid for ',idate

      cpcu_mg%grid = LFOgrid
      eod_mg%grid  = LFOgrid

      rc = GRIDAllocate(cpcu_mg)
      if (isError(this,7,GridError()) .ne. 0) return
      rc = GRIDAllocate(eod_mg)
      if (isError(this,7,GridError()) .ne. 0) return

      rc = regrid(cpcu(1),cpcu_mg(1),exchangeGrid)
      rc = regrid(cpcu(2),cpcu_mg(2),exchangeGrid)

      rc = regrid(eod,eod_mg,exchangeGrid,eodMethod=nearestNeighbor)
...
!     =======================================
      ...
      rc = computeDailyPrecip(eod_mg,LFOin(1:48),precip(1))
      rc = computeDailyPrecip(eod_mg,LFOin(25:72),precip(2))
...
!     =============================================
      ...
      correction%grid = LFOgrid
      rc = GRIDallocate(correction)
      if (isError(this,7,GridError()) .ne. 0) return
      correction(1)%eod = eod_mg%data
      correction(2)%eod = eod_mg%data

      call computeCorrection(precip(1),cpcu_mg(1),correction(1))
      call computeCorrection(precip(2),cpcu_mg(2),correction(2))
...
!         Otherwise, the HOD falls in the day-2 period [EOD,EOD+24]. 
        ...
        where (factor%data .eq. LFOgrid%undef) factor%data = 1.0

!       Apply the correction factors

        LFOout(hour)%precls = LFOin(hour)%precls * factor%data
        LFOout(hour)%preccon = LFOin(hour)%preccon * factor%data
        LFOout(hour)%precsno = LFOin(hour)%precsno * factor%data
...
!     =============================================
      ...
      timezone%eod  => correction(1)%eod
      timezone%grid%undef = correction(1)%grid%undef
...
!       -----------------------------------------
        ...
        do localHour = 0,2 
          ...
          hIndex = utcTime - 6 + 1
          call addResidual(timezone,correction,LFOin(hIndex),LFOout(hIndex))
...
!     ---------------
      ...
      namelist /corrector/ in_file, out_file, cpcu_file, cpcu_scale_file, &
                           eod_file, mask_file, exclude_africa, &
                           latitude_tapering, maskout, exchange_dlon, &
                           exchange_dlat
...
!       =========================
        ...
        select case (errorEvent)
          ...
          case (11)
            ...
            print *, 'correctPrecip (error#',code,'): error reading CPCU ', &
                     'scaling data'
        ...
        select case (errorEvent)
          ...
          case (2)
            ...
            print *, 'applyMask (error#',code,'): ', &
                     'CPCU_MASK_FILENAME environment variable is undefined'
          case (3)

            print *, 'applyMask (error#',code,'): ', &
                     'unable to access masking file: "',arg,'"'
          case (4)

            print *, 'applyMask (error#',code,'): ', &
                     'error reading masking file: "',arg,'"'
...
!     Module Public Error Handler
      ...
      end module CPCU_B_CorrectorMod
