      ...
      type UserInterfaceType
        ...
        character(len=1024) :: EODFilename
        character(len=1024) :: inLFOfNameTmplt
        character(len=1024) :: CPCUfNameTmplt
        character(len=1024) :: outLFOfNameTmplt
...
!     =================
      ...
      rc = CPCURead2Days(args%CPCUfNameTmplt,idate,0)
      if (isError(this,2,CPCUFileError()) .ne. 0) return

      rc = CPCUReadEOD(args%EODFilename)
...
!     ===================================
      ...
      rc = LFORead3Days(args%inLFOfNameTmplt,idate,63000)
...
!     ==================================
      ...
      rc = applyMask(LFOin(25:48),LFOout)
...
!     ==========================================
      ...
      rc = LFOWriteFile(ARGS%outLFOfNameTmplt,LFOout)
...
!******************************************************************************
      integer function applyMask(LFOin,LFOout)
!******************************************************************************
...
!          where the CPCU data is not reliable.
!
! Language: Fortran 90
! ---------
!
...
!     =====================
      ...
      call getenv('CPCU_MASK_FILENAME',filename)
      if (len_trim(filename) .le. 0) rc = 1
      if (isError(this,2,rc) .ne. 0) return
...
!     Set the corrected precip to
!     the original model value for
!     each hour where the mask is 
!     set to '1'.
!     ============================
...
!     Retrieve start/end dates (ccyymmdd)
!     ===================================

      call getarg(2,str)
      read(unit=str,fmt='(i10)') args%startDate
      if (args%startDate .lt. 19000000) rc = 1
      if (isError(this,2,rc,arg=str)) return

      call getarg(3,str)
      read(unit=str,fmt='(i10)') args%endDate
...
!     ===============================
      ...
      call getarg(4,args%inLFOfNameTmplt)
      call getarg(5,args%outLFOfNameTmplt)
      call getarg(6,args%CPCUfNameTmplt)
      call getarg(7,args%EODFilename)
...
!       ====================
        ...
        select case (errorEvent)
          ...
          case (1)
            ...
            print *, 'Usage: correctPrecip [config] [start date] [end date]', &
                     ' [input LFO filename] [output LFO filename]', &
                     ' [CPCU filename] [End-of-day filename]'
            print *, ' '
            print *, 'where'
            print *, ' '
            print *, '(1) start/end date format is ccyymmdd'
            print *, '(2) date/time in filenames must be specified as', &
                     ' replaceable tokens (e.g. %Y%m%d%H)'
            print *, ' '

          case (2)

            print *, 'getArgs (error#',code,'): bad date format'
            print *, 'getArgs: date = ',arg(1:len_trim(arg))
...
!     Module Public Error Handler
      ...
      end module CPCU_2_CorrectorMod
