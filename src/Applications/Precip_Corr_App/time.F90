...
!**********************************************************************
      type (PentadDataType) function tm_pentad(idate)
!**********************************************************************
      ...
      integer :: pentad,julian,year,offset,num_days

!     Determine the pentad index
!     ==========================

      call tm_string(idate,0,'%j',str)
      read(unit=str,fmt='(i)') julian

      year = idate / 10000

      if (tm_leapyear(year) .and. julian .gt. LEAP_DAY) then
        pentad = (julian - 2) / 5 + 1
        offset = mod(julian-2,5)
      else
        pentad = (julian - 1) / 5 + 1
        offset = mod(julian-1,5)
      endif
...
!     ===========================
      ...
      if (tm_leapyear(year) .and. pentad .eq. 12) num_days = 6
      if (num_days .eq. 6 .and. julian .eq. LEAP_DAY+1) offset = 5

      sdate = idate
      stime = 0
      call tm_inctime(sdate,stime,-offset*24,0,0)

      edate = sdate
      etime = 0
      call tm_inctime(edate,etime,(num_days-1)*24,0,0)

!     Save information and return
!     ===========================

      tm_pentad%index    = pentad
      tm_pentad%offset   = offset
      tm_pentad%date     = idate
      tm_pentad%sdate    = sdate
      tm_pentad%edate    = edate
      tm_pentad%julian   = julian
      tm_pentad%num_days = num_days

      end function tm_pentad
