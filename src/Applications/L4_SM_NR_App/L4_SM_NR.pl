...
#******************************************************************************
  ...
  my @inputs = qw/CLIM BCS_CLSM BCS_RTM LFO_INST LFO_TAVG LFO_CORR/;
  my @outputs = qw/LMC GPH TAVG3H INST1H TILE RC_OUT RESTART/;
...
#******************************************************************************
{
  ...
  system( "L4_SM_NR_genISO.pl -pc $pc " .
          "$config->{L4_SM_ROOT} " .
          "$config->{DATE} 0 " .
          hash2opt(%$dictionary)
...
# =======================
  ...
  $error_handle eq "runModel" and do {
    ...
    $error_code == 4 and do {
      $lh->$type(4,"Failed to acquire data for type " .
                                "\"$options{TYPE}\"");
      return;
    };

    $error_code == 5 and do {
      $lh->$type(5,"No data available for type " .
                                "\"$options{TYPE}\"");
      return;
    };
