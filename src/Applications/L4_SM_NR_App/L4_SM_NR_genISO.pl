...
# ==================================
  ...
  $output->execute("LMC");
  $output->execute("GPH");
  $output->execute("TAVG3H");
  $output->execute("INST1H");
...
# ======================================
  ...
  $dtr = $input->{"TAVG3H"}; $dtr or return $eh->fatal(1,TYPE=>"TAVG3H");
  ($tstart[1], $tend[1]) = $dtr->times();

  $dtr = $input->{"INST1H"}; $dtr or return $eh->fatal(1,TYPE=>"INST1H");
...
# ===========================================
  ...
  for (my $t = $tstart; $t <= $tend; $t+=1800) {
    ...
    genTAVG3H($t,$input,$output,$config);
    $eh->isError() and return $eh->traceback(2);

    genINST1H($t,$input,$output,$config);
    $eh->isError() and return $eh->traceback(2);
...
# $file = (findLastFile($time,$output->{GPH},$config))[1];
...
#******************************************************************************
sub genTAVG3H { CLOCK:  my $time = shift;
                DMGR:   my $input = shift;
                DMGR:   my $output = shift;
                CONFIG: my $config = shift;
#******************************************************************************
# English Name: Generate TAVG3H Granule
# -------------
#
# Purpose: Generates a time average (TAVG3H) granule for the specified
# -------- date/time if the input data exists. TAVG3H is one of the collection
#          types produced by the Land Data Assimilation System (LDAS).
#
# Language: Perl
# ---------
#
# Usage: $rc = genTAVG3H($time,$input,$output,$config);
# ------ $exception = $eh->isError();
#
# Interface:              Type   Access  Description
# ----------                     Intent
...
#
#                                        1: Data transaction file is missing
#                                           for input TAVG3H data.
#
#                                        2: Data transaction file is missing
#                                           for output TAVG3H data.
#
#                                        3: Error returned from called method.
#    
# Programmer: Joseph V. Ardizzone
...
# Modified:       Date           Author  Description
# ---------   
#           06/20/2020      J.Ardizzone  created.
#******************************************************************************

  FUNCTION_NAME: my $this = "genTAVG3H";
  ERROR_HANDLER: my $eh = ERROR::Handler->new(\&error_handler,$this);

# Return if there is no TAVG3H input
# data for the specified time.
# ===============================

  $input->{TAVG3H} or return $eh->fatal(1,TYPE=>"TAVG3H");
  $output->{TAVG3H} or return $eh->fatal(2,TYPE=>"TAVG3H");

  DTR: my $dtr = $input->{TAVG3H};

  METADATA:  my $metadata = $config->clone(COLLECTION_ID=>"TAVG3H");
  FILENAMES: my @files = $dtr->files();
  FILETMPLT: my ($file)  = grep /\.bin/, @files;
  FILENAME:  my $pathname = $config->time_interp($file,$time);
  ...
  ($output->stat("TAVG3H",CLOCK=>$time))[0] and return 1;

  my $datetime = $time->datetime;
  $eh->comment( "Generating TAVG3H granules for $datetime");

# Create the TAVG3H granule
# =========================
...
#******************************************************************************
sub genINST1H { CLOCK:  my $time = shift;
                DMGR:   my $input = shift;
                DMGR:   my $output = shift;
                CONFIG: my $config = shift;
#******************************************************************************
# English Name: Generate INST1H Granule
# -------------
#
# Purpose: Generates a time average (INST1H) granule for the specified
# -------- date/time if the input data exists. INST1H is one of the collection
#          types produced by the Land Data Assimilation System (LDAS).
#
# Language: Perl
# ---------
#
# Usage: $rc = genINST1H($time,$input,$output,$config);
# ------ $exception = $eh->isError();
#
# Interface:              Type   Access  Description
# ----------                     Intent
...
#
#                                        1: Data transaction file is missing
#                                           for input INST1H data.
#
#                                        2: Data transaction file is missing
#                                           for output INST1H data.
#
#                                        3: Error returned from called method.
#    
# Programmer: Joseph V. Ardizzone
...
#******************************************************************************
  ...
  FUNCTION_NAME: my $this = "genINST1H";
  ERROR_HANDLER: my $eh = ERROR::Handler->new(\&error_handler,$this);

# Return if there is no INST1H input
# data for the specified time.
# ===============================

  $input->{INST1H} or return $eh->fatal(1,TYPE=>"INST1H");
  $output->{INST1H} or return $eh->fatal(2,TYPE=>"INST1H");

  DTR: my $dtr = $input->{INST1H};

  METADATA:  my $metadata = $config->clone(COLLECTION_ID=>"INST1H");
  FILENAMES: my @files = $dtr->files();
  FILETMPLT: my ($file)  = grep /\.bin/, @files;
  FILENAME:  my $pathname = $config->time_interp($file,$time);
  ...
  ($output->stat("INST1H",CLOCK=>$time))[0] and return 1;

  my $datetime = $time->datetime;
  $eh->comment( "Generating INST1H granules for $datetime");

# Create the INST1H granule
# =========================
...
# genTAVG3H() error handle
# ========================

  $error_handle eq "genTAVG3H" and do {
...
# genINST1H() error handle
# ========================

  $error_handle eq "genINST1H" and do {
