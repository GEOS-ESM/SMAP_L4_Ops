...
# ====================================
  ...
  my ($is_complete) = getEventTimes("L4_SM is complete",@log);
...
# ========================
  ...
  my ($end_time)    = (getEventTimes('Exiting L4_SM.pl',@log))[-1];

  my @model = (
    getEventTimes("Submitting the model batch job",@log),
    getEventTimes("Model successfully completed",@log)
  );

  my @genPCTL = (
    getEventTimes("Generating percentile data", @log),
    getEventTimes("Exiting genPCTL", @log)
  );

  my @genISO = (
    getEventTimes("Begin Generation of SMAP Granules", @log),
  ...
  @wait = getEventTimes('Waiting for data type: "LFO_INST"', @log);
  push @wait, $wait[-1] + 1800 if @wait;
  my @lfo_inst = ($start_time, $model[0]);
  @lfo_inst = ($wait[-1], $model[0]) if @wait;

  @wait = getEventTimes('Waiting for data type: "LFO_TAVG"', @log);
  push @wait, $wait[-1] + 1800 if @wait;
  my @lfo_tavg = ($start_time, $model[0]);
  @lfo_tavg = ($wait[-1], $model[0]) if @wait;

  @wait = getEventTimes('Waiting for data type: "LFO_CORR"', @log);
  push @wait, $wait[-1] + 1800 if @wait;
  my @lfo_corr = ($start_time, $model[0]);
  @lfo_corr = ($wait[-1], $model[0]) if @wait;

  @wait = getEventTimes('Waiting for data type: "SPL1CTB"', @log);
  push @wait, $wait[-1] + 1800 if @wait;
  my @spl1ctb = ($start_time, $model[0]);
  @spl1ctb = ($wait[-1], $model[0]) if @wait;

  @wait = getEventTimes('Waiting for data type: "SPL2SMA"', @log);
  push @wait, $wait[-1] + 1800 if @wait;
  my @spl2sma = ($start_time, $model[0]);
  @spl2sma = ($wait[-1], $model[0]) if @wait;

  @wait = getEventTimes('Waiting for data type: "SPL2SMAP"', @log);
  push @wait, $wait[-1] + 1800 if @wait;
  my @spl2smap = ($start_time, $model[0]);
  @spl2smap = ($wait[-1], $model[0]) if @wait;
...
# ===================
  ...
  writeLineMeta(9,10,2,($start_time, $spl2smap[0]));
  writeLineMeta(9,11,3,@spl2smap);

  writeLineMeta(8,8,2,($start_time, $spl2sma[0]));
  writeLineMeta(8,9,3,@spl2sma);

  writeLineMeta(7,6,2,($start_time, $spl1ctb[0]));
  writeLineMeta(7,7,3,@spl1ctb);

  writeLineMeta(6,4,2,($start_time, $lfo_corr[0]));
  writeLineMeta(6,5,3,@lfo_corr);

  writeLineMeta(5,2,2,($start_time, $lfo_tavg[0]));
  writeLineMeta(5,3,3,@lfo_tavg);

  writeLineMeta(4,0,2,($start_time, $lfo_inst[0]));
  writeLineMeta(4,1,3,@lfo_inst);

  writeLineMeta(3,12,3,@model);
  writeLineMeta(2,13,3,@genPCTL);
  writeLineMeta(1,14,3,@genISO);
  writeLineMeta(0,15,3,@export);
...
# print "\@    subtitle color 1\n";
  ...
  print "\@    yaxis  ticklabel 2, \"genPCTL\"\n";
  print "\@    yaxis  tick major 3,                3\n";
  print "\@    yaxis  ticklabel 3, \"runModel\"\n";
  print "\@    yaxis  tick major 4,                4\n";
  print "\@    yaxis  ticklabel 4, \"LFO_INST\"\n";
  print "\@    yaxis  tick major 5,                5\n";
  print "\@    yaxis  ticklabel 5, \"LFO_TAVG\"\n";
  print "\@    yaxis  tick major 6,                6\n";
  print "\@    yaxis  ticklabel 6, \"LFO_CORR\"\n";
  print "\@    yaxis  tick major 7,                7\n";
  print "\@    yaxis  ticklabel 7, \"SPL1CTB\"\n";
  print "\@    yaxis  tick major 8,                8\n";
  print "\@    yaxis  ticklabel 8, \"SPL2SMA\"\n";
  print "\@    yaxis  tick major 9,                9\n";
  print "\@    yaxis  ticklabel 9, \"SPL2SMAP\"\n";
