...
# ==============================
  ...
  $config->{L4CMOD_version} = $meta->h5dump("-a L4C_SW_VERSION " . 
                                                                    $mod_file);
# $config->{L4CMOD_creationDate} = $t->datetime . ".000Z";
  $config->{L4CMOD_creationDate} = $meta->h5dump("-a L4_C_PRODUCTION_DATETIME " .  $mod_file);

# $config->{L4CMOD_creationDate} =~ s/Z$/\.000Z/;
  $config->{L4CMOD_DOI} = $meta->h5dump("-a DOI " . $mod_file);
...
# ===============================
  ...
  foreach my $pathname (@array) {
    ...
    my $name         = basename $pathname;
    my $creationDate = $info{PRODUCTIONDATETIME};
    my $version      = $info{LOCALVERSIONID};
    my $DOI          = $info{DOI};
