#!/bin/sh
  ...
  while [ $idate -le $edate ]; do
    ...
    L4.pl -export -st $stream_type -sn $stream_name -task export_special/jpl -date $idate
    ...
    while [ $numpdr -gt 150 ]; do
      sleep 300
      L4.pl -purge -st $stream_type -sn $stream_name -task export_special/jpl
      numpdr=`/bin/ls $EXPDIR | wc -l`
    done
