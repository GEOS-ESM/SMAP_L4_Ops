...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
                              ...
                              xsi:schemaLocation="http://hdfgroup.org/HDF5/XML/schema/HDF5-File http://www.hdfgroup.org/HDF5/XML/schema/HDF5-File.xsd"
    ...
    <!-- start/end dates for series -->
    <xsl:variable name="sdate">
        <xsl:choose>
            <xsl:when test="contains($fi, 'aup')">
                <xsl:value-of select="'2015-03-31T01:30:00.000z'"/>
            </xsl:when>
            <xsl:when test="contains($fi, 'gph')">
                <xsl:value-of select="'2015-03-31T00:00:00.000z'"/>
            </xsl:when>
            <xsl:when test="contains($fi, 'lmc')">
                <xsl:value-of select="'2015-03-31T00:00:00.000z'"/>
            </xsl:when>
            <xsl:when test="contains($fi, 'mdl')">
                <xsl:value-of select="'2015-04-13T00:00:00.000z'"/>
            </xsl:when>
            <xsl:otherwise>
                <xsl:value-of select="''"/>
            </xsl:otherwise>
    ...
    <xsl:variable name="edate">
        <xsl:choose>
            <xsl:when test="contains($fi, 'aup')">
                <xsl:value-of select="'2021-01-01T01:29:59.999z'"/>
            </xsl:when>
            <xsl:when test="contains($fi, 'gph')">
                <xsl:value-of select="'2020-12-31T23:59:59.999z'"/>
            </xsl:when>
            <xsl:when test="contains($fi, 'lmc')">
                <xsl:value-of select="'2021-01-01T01:29:59.999z'"/>
            </xsl:when>
                            ...
                            <gmd:MD_DataIdentification>
                                <gmd:citation>
                                    <xsl:call-template name="citation">
                                        ...
                                        <xsl:with-param name="series" select="hdf5:Attribute[@Name='SMAPShortName']/hdf5:Data/hdf5:DataFromFile"/>
                                        <xsl:with-param name="other" select="'The Beta Release of the SMAP Level 4 Science Processing Software.'"/>
                                    </xsl:call-template>
                                </gmd:citation>
                                <xsl:call-template name="type">
                                    <xsl:with-param name="tag">gmd:abstract</xsl:with-param>
