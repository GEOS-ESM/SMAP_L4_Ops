...
setenv GEOSBIN <L4_SM_STREAM_directoryID>/bin/GEOSldas/install/bin
...
cp -p <L4_SM_MODEL_OUTPUT_DIR>/rc_out/<L4_SM_STREAM_version>.ldas_smapL4SMlmc.bin <L4_SM_STREAM_version>.ldas_smapL4SMlmc.%Y%m%d_0000z.bin
...
$GEOSBIN/esma_mpirun -np 112 $GEOSBIN/GEOSldas.x
...
mkdir -p <L4_SM_MODEL_OUTPUT_DIR>/cat/ens0000/Y%Y/M%m

foreach t ( 0130 0430 0730 1030 1330 1630 1930 2230 )

  mv <L4_SM_STREAM_version>.SMAP_L4_SM_gph.%Y%m%d_${t}z.bin <L4_SM_MODEL_OUTPUT_DIR>/cat/ens0000/Y%Y/M%m
  mv <L4_SM_STREAM_version>.tavg3_1d_lnr_Nt.%Y%m%d_${t}z.bin <L4_SM_MODEL_OUTPUT_DIR>/cat/ens0000/Y%Y/M%m
...
foreach i (`seq 1 1 23`)
  ...
  mv <L4_SM_STREAM_version>.inst1_1d_lnr_Nt.%Y%m%d_${t}z.bin <L4_SM_MODEL_OUTPUT_DIR>/cat/ens0000/Y%Y/M%m

end

mkdir -p <L4_SM_MODEL_OUTPUT_DIR>/cat/ens0000/Y%%Y/M%%m

mv <L4_SM_STREAM_version>.inst1_1d_lnr_Nt.%%Y%%m%%d_0000z.bin <L4_SM_MODEL_OUTPUT_DIR>/cat/ens0000/Y%%Y/M%%m
